...
void SetPlayerStop(CHARACTER* c)
{
	...
	if (c->Object.Type == MODEL_PLAYER)
	{
		if (c->Helper.Type == MODEL_HELPER + 37 && !c->SafeZone)
		{
			...
			if (GetBaseClass(c->Class) == CLASS_RAGEFIGHTER)//·¹ÀÌÁöÆÄÀÌÅÍÀÌ¸é
			{
...
void DeadCharacter(CHARACTER* c, OBJECT* o, BMD* b) // Thời gian chết
{
	if (c->Dead <= 0) return;
	...
	// Giảm thời gian chết từ 1.0 xuống 0.3 giây
	float RotTime = 0.7f;
	if (c->Rot >= RotTime)
	{
		if (o->Type != MODEL_MONSTER01 + 116)
		{
			// Giảm Alpha nhanh hơn để biến mất sớm
			o->Alpha = 1.f - (c->Rot - RotTime) * 3;
		}

		if (o->Alpha >= 0.1f)
		{
			// Đẩy quái xuống nhanh hơn
			o->Position[2] -= 1.0f;
		}
		else if (c != Hero)
		{
...
void MoveCharacterPosition(CHARACTER* c)
{
	...
	{
		o->Position[2] += (-absf(sinf(o->Timer)) * 10.f + 10.f) * FPS_ANIMATION_FACTOR; // Fix Con Rồng Lorencia
	}
	o->Timer += 0.10f * FPS_ANIMATION_FACTOR; // Cập nhật Timer theo FPS
}
